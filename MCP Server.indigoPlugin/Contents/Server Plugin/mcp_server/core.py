...
class MCPServerCore:
    ...
    def _schedule_component_logging(self) -> None:
        ...
        log_thread = threading.Thread(
            target=delayed_logging, daemon=True, name="MCP-ComponentLog-Thread"
        )
        log_thread.start()

    def _log_components_from_objects(
        self, tools: dict, resources: dict, prompts: dict
    ) -> None:
        ...
        if tools:
            ...
            self.logger.info(
                f"Published MCP Tools ({len(tool_names)}): {', '.join(tool_names)}"
            )
        else:
        ...
        if resources:
            ...
            if resource_uris:
                self.logger.info(
                    f"Published MCP Resources ({len(resource_uris)}): {', '.join(resource_uris)}"
                )
            else:
                self.logger.info(
                    f"Published MCP Resources ({len(resources)}): {', '.join(resources.keys())}"
                )
        else:
        ...
        if prompts:
            ...
            self.logger.info(
                f"Published MCP Prompts ({len(prompt_names)}): {', '.join(prompt_names)}"
            )
        else:
    ...
    async def _log_published_components(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self.logger.debug(
                    f"FastMCP internal API failed, trying HTTP fallback: {internal_error}"
                )
    ...
    async def _log_components_via_http(self) -> None:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                try:
                    async with session.post(
                        url, json=tools_request, timeout=timeout
                    ) as response:
                ...
                try:
                    async with session.post(
                        url, json=resources_request, timeout=timeout
                    ) as response:
                        ...
                        if (
                            ...
                            resource_uris = [
                                res["uri"]
                                for res in resources_result["result"]["resources"]
                            ]
                            self.logger.info(
                                f"Published MCP Resources ({len(resource_uris)}): {', '.join(resource_uris)}"
                            )
                ...
                try:
                    async with session.post(
                        url, json=prompts_request, timeout=timeout
                    ) as response:
                        prompts_result = await response.json()
                        if (
                            "result" in prompts_result
                            and "prompts" in prompts_result["result"]
                        ):
                            prompt_names = [
                                prompt["name"]
                                for prompt in prompts_result["result"]["prompts"]
                            ]
                            self.logger.info(
                                f"Published MCP Prompts ({len(prompt_names)}): {', '.join(prompt_names)}"
                            )
            ...
            self.logger.debug(
                "Published MCP Components: Unable to query (aiohttp not available)"
            )
        except Exception as e:
            self.logger.debug(
                f"Published MCP Components: Unable to query via HTTP ({e})"
            )
